...
<template>
  <VDialog :width="$vuetify.display.smAndDown ? 'auto' : 650" persistent :model-value="props.isEditOpen"
    ...
    <VCard class="pa-sm-9 pa-5">
      ...
      <VCardText>
        ...
        <VForm ref="refForm" @submit.prevent.stop="onFormSubmit">
          <VRow>
            <VCol cols="12" lg="12">
              <VTextField v-model="couponData.name" :label="t('forms.name')" :rules="[requiredValidator]" />
            </VCol>
            <VCol cols="12" lg="12">
              <VTextField v-model="couponData.code" :label="t('forms.code')" :rules="[requiredValidator]" />
            </VCol>
            <VCol cols="12" lg="12">
              <VRow>
                <VCol cols="12" sm="3">
                  <VSwitch v-model="couponData.is_percent" :label="t('forms.is_percent')" />
                </VCol>
                <VCol cols="12" sm="9">
                  <VTextField v-model="couponData.discount_amount_percent"
            ...
            <VCol cols="12" lg="12">
              <VTextField v-model="couponData.min_applied_amount" :label="t('forms.min_applied_amount')"
                :rules="[requiredValidator]" type="number" min="0" />
            </VCol>
            <VCol cols="12" lg="12">
              <VTextField v-model="couponData.max_discount" :label="t('forms.max_discount')"
                :rules="[requiredValidator]" type="number" min="0" />
            </VCol>
            <VCol cols="12" lg="12">
              <VSwitch v-model="couponData.for_clients_only" :label="t('forms.for_clients_only')" />
              <div v-if="couponData.for_clients_only">
                <VSelect v-model="couponData.client_ids" :items="customers.value" :label="t('forms.customers')"
                  item-title="name" item-value="id" multiple :rules="[requiredValidator]" />
              </div>
            </VCol>
            <VCol cols="12" lg="12">
              <VSwitch v-model="couponData.is_by_product" :label="t('forms.is_by_product')" />
              <div v-if="couponData.is_by_product">
                <VSelect v-model="couponData.product_ids" :items="products.value" :label="t('forms.products')"
                  item-title="name_ar" item-value="id" multiple :rules="[requiredValidator]" />
              </div>
            </VCol>
            <VCol cols="12" lg="12">
              <VSwitch v-model="couponData.is_by_country" :label="t('forms.is_by_country')" />
              <div v-if="couponData.is_by_country">
                <VSelect v-model="couponData.country_ids" :items="countries.value" :label="t('forms.countries')"
                  item-title="name_ar" item-value="id" multiple :rules="[requiredValidator]" />
              </div>
            </VCol>
            <VCol cols="12">
              <VSwitch v-model="couponData.is_by_city" :label="t('forms.is_by_city')" />
              <div v-if="couponData.is_by_city">
                <VSelect v-model="couponData.city_ids" :items="cities.value" :label="t('forms.cities')"
                  item-title="name_ar" item-value="id" multiple :rules="[requiredValidator]" />
              </div>
            </VCol>
            <VCol cols="12" lg="12">
              <VSwitch v-model="couponData.is_by_category" :label="t('forms.is_by_category')" />
              <div v-if="couponData.is_by_category">
                <VSelect v-model="couponData.category_parent_ids" :items="categories.value"
                  :label="t('forms.categories')" item-title="type_ar" item-value="id" multiple
                  :rules="[requiredValidator]" />
              </div>
            </VCol>
            <VCol cols="12" lg="12">
              <VSwitch v-model="couponData.is_by_subcategory" :label="t('forms.is_by_subcategory')" />
              <div v-if="couponData.is_by_subcategory">
                <VSelect v-model="couponData.category_child_ids" :items="subCategories.value"
                  :label="t('forms.sub_categories')" item-title="type_ar" item-value="id" multiple
                  :rules="[requiredValidator]" />
              </div>
            </VCol>
            <VCol cols="12" lg="12">
              <VTextField v-model="couponData.use_times_per_user" :label="t('forms.use_times_per_user')"
                :rules="[requiredValidator]" />
            </VCol>
            <!-- <VCol cols="12" lg="12"> -->
              <!-- <AppDateTimePicker
            ...
            <div class="v-col-lg-12 v-col-12">
                ...
                <div class="v-input__control">
                  <div
                    ...
                    <div class="v-field__field" data-no-activator=""><!----><label class="v-label v-field-label"
                        ...
                        <input v-model="couponData.expire_at" type="datetime-local" id="expire_at" name="expire_at" class="v-field__input"><!----></div>
                    <!----><!---->
              ...
              <VCol cols="12" lg="6">
                <VSwitch v-model="couponData.is_active" :label="t('forms.is_active')" />
              </VCol>
              <VCol cols="12" lg="6">
                <VSwitch v-model="couponData.is_for_all" :label="t('forms.is_for_all')" />
              </VCol>
              <VCol cols="12" lg="12" md="6">
                <VTextField v-model="couponData.foodics_integrate_id" label="foodics_integrate_id" />
              </VCol>
